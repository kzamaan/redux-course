				...
				{bookingList.length > 0 ? (
					bookingList.map((booking) => (
						<tr key={booking.id} className="lws-bookedTable text-black">
							...
							<td className="px-6 py-4 text-center">
								<span className="lws-bookedClass"> {booking.flight_class} </span>
							</td>
							<td className="px-6 py-4 text-center">
								<div className="flex justify-center gap-4">
									<button className="lws-remove" onClick={() => handleRemoveBooking(booking.id)}>
										<svg
					...
					<tr className="lws-bookedTable text-black">
						<td className="px-6 py-4" colSpan="6">
							<div className="flex items-center space-x-3 justify-center">
								<p className="lws-bookedFrom">No Booking Found!</p>
							</div>
						</td>
					</tr>
				)}
