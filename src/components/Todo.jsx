...
export default function Todo({ todo }) {
	...
	const handlerStatusChange = (todoId) => {
			...
			<div className="flex justify-start items-center p-2 hover:bg-gray-100 hover:transition-all space-x-4 border-b border-gray-400/20 last:border-0">
				<div
					className={`rounded-full bg-white border-2 w-5 h-5 flex flex-shrink-0 justify-center items-center mr-2 ${
						completed ? 'border-green-500 focus-within:border-green-500' : 'border-gray-400'
					}`}>
					<input
						type="checkbox"
						checked={completed}
						onChange={() => handlerStatusChange(id)}
				...
				<div
					...
					onClick={() => handlerColorChange(id, 'green')}></div>

				<div
					className={`flex-shrink-0 h-4 w-4 rounded-full border-2 ml-auto cursor-pointer hover:bg-yellow-500 border-yellow-500 ${
						color === 'yellow' && ' bg-yellow-500'
					}`}
					onClick={() => handlerColorChange(id, 'yellow')}></div>

				<div
					className={`flex-shrink-0 h-4 w-4 rounded-full border-2 ml-auto cursor-pointer hover:bg-red-500 border-red-500 ${
						color === 'red' && ' bg-red-500'
					}`}
					onClick={() => handlerColorChange(id, 'red')}></div>
